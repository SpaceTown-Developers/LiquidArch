...
if ( SERVER ) then
	...
	function Plugin:PlayerInitialSpawn( Ply ) -- Check rank
		...
		if ( Rank and !LA.Ranks[Rank] ) then
			LA:Message( Ply, LA.Colors.White, "You are supposed to be a(n) ",LA.Colors.Blue, Rank, LA.Colors.White, ", that rank is missing so your now a Guest.." )
			Rank = "Guest"
		elseif ( Rank ) then
