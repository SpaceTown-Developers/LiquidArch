...
function LA:StringToTime( Input )
	if !Input then
		return 5 * Times.m
	end
	...
	if Time and tostring( Time ) == Input then
		return Time * Times.m
	elseif !string.match( Input, "^([0-9]+)([a-zA-Z])" ) then
		return nil
	end
	
	Time = 0
			
	string.gsub( string.lower( Input ), "([0-9]+)([a-zA-Z])", function( Duration, Unit )
